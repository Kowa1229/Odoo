...
class HrInsurance(models.Model):
    ...
    # name = fields.Char('')
    name = fields.Many2one('hr.insurance.type', string='Insurance Type', required=True, help="The type of the insurance.")
    ...
    insurance_company = fields.Many2one('res.partner', string='Company Name', default=False)
    ...
    def action_draft(self):
        if self.state not in ['pending', 'cancel', 'renew']:
            raise ValidationError('Insurance state must be "Pending" or "Cancel" or "Renew" in order to reset to Draft.')
    ...
    def action_running(self):
        if self.state not in ['pending', 'renew']:
            raise ValidationError('Insurance state must be "Pending" or "Renew" in order to Set to Running.')
    ...
    def action_renew(self):
        if self.state not in ['running', 'expired']:
            raise ValidationError('Insurance state must be "Running" or "Expired" in order to Renew state.')
...
class HrInsuranceType(models.Model):
    ...
    cover_ids = fields.Many2many('hr.insurance.cover.type', string='Insurance Cover Types')
...
class HrInsuranceCoverType(models.Model):
    _name = 'hr.insurance.cover.type'
    _description = 'Insurance Cover Types'

    name = fields.Char('Description', required=True, help="Insert the description of the Insurance Cover Type.")
